  ...
  <div class="container-artifacts-associated">
      ...
      <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="10" v-loading="loading">
            ...
            <el-tree ref="tree" node-key="id" show-checkbox default-expand-all highlight-current class="tree_height" :data="array" :props="defaultProps" @check-change="handleCheckChange"></el-tree>
          </el-col>
    ...
    <el-dialog title="字段映射" :visible.sync="centerDialogVisible" width="50%" center>
      ...
      <div v-if="showResult" class="result center">
        ...
        <p>本项目共 <span>{{totalCount}}</span> 个构件，本次您成功关联了 <span>{{relation}}</span> 个构件，失败 <span>{{fail}}</span> 个；</p>
