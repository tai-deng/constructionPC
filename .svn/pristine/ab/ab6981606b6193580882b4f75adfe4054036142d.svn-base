...
export const getResourceList = (that, GetResourceBySpInfo) => {
    ...
    GetResourceBySpInfo({ projectid: projectid, building: that.formInline.BuildName, Specialty: that.formInline.Specialty, Floor: that.formInline.Floor }).then(
        res => {
            // that.$toast.clear();
