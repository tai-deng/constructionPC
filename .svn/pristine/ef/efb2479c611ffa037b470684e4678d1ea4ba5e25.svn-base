...
export default {
  ...
  methods: {
    ...
    async AuthenticationInfoByTaskId(id){
      ...
      if(res.StatusCode === 200){
        // console.log(this.imgList1)
        // this.imgList1[0].url = res.Detiel[0].PicturePath.split(',')[0]
        // this.imgList2[0].url = res.Detiel[0].PicturePath.split(',')[1]
    ...
    async GetAddModelAuthentication(data={}){
      ...
      let res = await this.Request(AddModelAuthentication(
        {TaskId, Percentage, PicturePath, UploadTime}))      
     if(res.StatusCode=== 200){
        this.$message({
          type : 'success',
          message: res.Message,
