    ...
    <div class='model_main_all'>
       ...
       <!-- 分享模态框 -->
            ...
            <div class="model_toget_all_div">
                ...
                <div class="model_body">
                    ...
                    <div class="model_body_footer">
                        <el-button class="model_body_footer_btn" :disabled="arr.length<2?true:false" @click="getTogetherBtn">确认合并</el-button>
                    </div>
        ...
        <el-dialog  title="合并提示" :visible.sync="togetherVisible" width="30%"  center>
            <p>请输入集成模型名称名称<el-input v-model="togetherName"></el-input></p>
            <span slot="footer" class="dialog-footer">
...
<script>
import {AddSingBuilding,GetSingBuildingList,UpdateSingBuilding,DeleteSingBuilding,UploadBIMFaceAddModelFile,GetList,GetDetailList,MergeFile} from '@/api/api.js'
export default {
    data(){
        return{
    ...
    methods:{
        ...
        //根据项目ID和建筑单体ID获取模型文件ID和名称列表-------现在是默认页面加载的时候执行--用于建筑单体显示该条信息的模型
        GetList(){
         ...
         //根据项目ID和建筑单体ID获取详情
        GetDetailList(){
            this.Request(GetDetailList({projectId:localStorage.getItem('projectid'),buildingId:1})).then(res=>{
                console.log('详情:',res);
            })
        },
        ...
        getTogetherAll(){
            console.log('发起合并'); //{TaskIds:TaskIds},post) sources-集成模型文件列表 config-集成模型配置
            // this.Request(MergeFile({projectId:localStorage.getItem('projectid'),name:this.togetherName,})).then(res=>{
            //     console.log('详情:',res);
            // })
