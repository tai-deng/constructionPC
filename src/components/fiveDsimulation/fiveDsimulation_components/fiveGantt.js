...
export const getFiveList = (that, init) => {
  ...
  GetFiveDSchedulePlanList(par).then(res => {
    ...
    if (res.StatusCode == 200 && res.Detiel) {
      let data = [];
      let links = [];
...
export const educeProject = that => {
  ...
  GetFiveDExport({
    ...
    if (res.StatusCode == 200) {
      ...
      that.educe.BuildingName = '';
      that.educe.LevelName = '';
      that.educe.BaseStartTime = '';
      that.educe.BaseFinishTime = '';
    }
    that.$message({
      type: "warning",
      message: res.Message,
